...
void insert_payload(binary_t &binary, binary_t const &payload) {
  ...
  IMAGE_SECTION_HEADER sect_hdr = {
    .Name = ".bin",
    .Misc = {.VirtualSize = static_cast<std::uint32_t>(payload.size()) },
    .VirtualAddress = align(last_sect_hdr->VirtualAddress + last_sect_hdr->Misc.VirtualSize, sect_align),
    .SizeOfRawData = static_cast<std::uint32_t>(payload.size()),
    .PointerToRawData = static_cast<std::uint32_t>(binary.size()),
    .PointerToRelocations = 0,
    .PointerToLinenumbers = 0,
    .NumberOfRelocations = 0,
    .NumberOfLinenumbers = 0,
    .Characteristics = IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_INITIALIZED_DATA | 0x1337
  };

  auto const &[sect_hdr_start, sect_hdr_end] = mem_bounds(&sect_hdr);
  std::uint64_t const insert_offset = reinterpret_cast<std::uint64_t>(last_sect_hdr + 1) - reinterpret_cast<std::uint64_t>(binary.data());
  ...
  nt_hdr->OptionalHeader.SizeOfImage += align(static_cast<std::uint32_t>(payload.size()) + sizeof(sect_hdr), sect_align);
  nt_hdr->OptionalHeader.SizeOfInitializedData += align(static_cast<std::uint32_t>(payload.size()), sect_align);
...
binary_t generator::generate() const {
  ...
  insert_payload(result, payload_);
  return result;
}
